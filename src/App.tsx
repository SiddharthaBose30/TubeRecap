...
function App() {
  ...
  const [summaryPoints, setSummaryPoints] = useState<
    { summary: string; links: { name: string; url: string; snippet: string }[] }[]
  >([]);
  ...
  const summarizeTranscript = async (transcriptChunks: string[], initialRequest: string) => {
    ...
    try {
      const response = await fetch(
        'https://tuberecapopenaiapis.azurewebsites.net/api/HttpTrigger1?code=4HPNm4sXWrZyDghWMT56WICdvilYYeNklONTt2GmEuzhAzFuP5lM1g%3D%3D',
        {
          method: 'POST',
          headers: {
      ...
      setSummaryPoints(data.summaryPoints); // Assumes summaryPoints contain both summary and links
    } catch (error) {
    ...
    <div className="container">
      <div className="header">
        <img
          ...
          style={{ width: '49px', height: '63.5px' }}
        />
        <p
          style={{
            marginTop: '20px',
      ...
      {buttonVisible && (
        <button id="button1" style={{ marginLeft: '20px', marginTop: '0px' }}>
          Summarize
        </button>
      )}
